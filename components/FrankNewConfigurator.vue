  ...
  <div class="flex-container" style="margin:240px 0 0 240px;">
    <div class="configurator">
      <div class="checkbox-row" v-for="(row, rowIndex) in rows" :key="rowIndex">
            ...
            <!--Position-->
            <div>
            <h3 v-show="rowIndex === 0 && colIndex === 0"> Pos. </h3>
...
export default {
  ...
  methods: {
    ...
    addRow() {
       ...
       const letztezeile = this.rows.length;
       const letztespalte = this.rows.length;
       
   /*    this.rows[1].splice(letztespalte,0,
      {
            position:letztezeile,
            keynr:1,
            checked:false
        })  */
      
        console.log('letzte Zeile ' + letztezeile);
        console.log('letzte Spalte ' + letztespalte);
    ...
    addCheckbox() {
       ...
       const letztezeile = this.rows[0].length;
       const letztespalte = this.rows[1].length;
    
      /*      this.rows[0].splice(letztezeile,0,
       {
            position: this.rows[0].length+1,
            doorDesignation: "",
            doorquantity: 1,
            type: "",
            outside: "",
          ...
          for (var i=0;i<letztezeile;i++){
      
            this.rows[1].splice(letztespalte,0,
      {
            position:i,
            keynr:letztespalte+1,
            checked:false
        }) 
      }
        ...
        console.log('letzte Zeile '+letztezeile);
        console.log('letzte Spalte '+letztespalte);
        //console.log(this.rows[1][1].keynr);
    },
    ...
    async saveInstallation() {
      ...
      const jsonstringrows = JSON.stringify(this.rows[0]);
      console.log(jsonstringrows);

      //const jsonstringkeys = JSON.stringify(this.keys);
      //console.log(jsonstringkeys);

      //const jsonstringmatrix = JSON.stringify(this.matrix);
      //console.log(jsonstringmatrix);

/*       const queryresultposition = await $fetch('/api/sqlpostmatrix?ID=' + '12345' , {
        method: 'post',
        body: jsonstringmatrix
